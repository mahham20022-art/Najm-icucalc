...
SeizureMeter is a Flutter web PWA, built as a sister app to
[Najm ICUCalc](https://mahham20022-art.github.io/Najm-icucalc/), sharing the
same dark "monitor-blue" design language, card layout and GitHub Pages
deployment model.
...
> ⚠️ **Medical disclaimer.** SeizureMeter is a clinical decision-support aid
> based on published seizure semiology literature. It estimates
...
2. **Localization Explorer** — interactive cards for Temporal, Frontal,
   Parietal, Occipital, Insular and Generalized onset, each with typical
   aura, semiology, EEG and MRI findings, and differentials.
3. **Seizure Library** — a searchable reference of 21 seizure types,
   reflex epilepsies and epilepsy syndromes (including PNES and convulsive
   syncope as mimics).
...
SeizureMeter deliberately reuses ICUCalc's palette and shapes rather than
...
lib/
  ...
  core/
    ...
    widgets/                   Shared cards, chips, bars, wizard chrome
  data/
    models/                    Plain data classes + enums
    repositories/               Static reference content (library, localization, pearls)
  domain/
    scoring/                    Evidence weights + the scoring engine
  features/
    home/                       Home screen (4 navigation cards)
...
```bash
flutter build web --release --base-href /Najm-icucalc/seizuremeter/
```
...
[`.github/workflows/deploy-seizuremeter.yml`](../.github/workflows/deploy-seizuremeter.yml)
at the repository root: on every push to `main` touching `seizuremeter/**`,
it runs `flutter analyze`, builds the release web bundle with the
`/Najm-icucalc/seizuremeter/` base href, and publishes `build/web` to the
`seizuremeter/` subfolder of the `gh-pages` branch (`keep_files: true`, so it
never touches the other apps already living on that Pages site, including
ICUCalc itself at the site root).

Once merged, the app is served at:

```
https://mahham20022-art.github.io/Najm-icucalc/seizuremeter/
```

To trigger a deploy manually, run the "Deploy SeizureMeter to GitHub Pages"
